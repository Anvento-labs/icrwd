"""
MongoDB helpers for the `support` receipt proof pipeline.

This ports the logic used by `lambdas/devyansh-lambda/services/mongodb_campaigns.py`,
but exposes only the functions needed by the active proof pipeline:
- duplicate detection (receipt_hash)
- campaign rules + merchant->campaign resolution
- user/block + membership checks
- multiproof sessions (proof_sessions collection)
- receipt upload history writes

Note: this intentionally preserves the devyansh-lambda collection split via
env vars (handled by `app.receipt_config` at the call sites).
"""
...
def get_active_crwds(
    ...
    crwds = list(cursor)
    logger.info(
        "get_active_crwds | database=%s | collection=%s | count=%s",
        database,
        crwds_collection,
        len(crwds),
    )
...
def find_crwd_by_merchant_name(
    ...
    best_crwd: Optional[Dict[str, Any]] = None
    best_score = 0.0
...
def get_campaign_rules(
    ...
    Otherwise derive from crwds: date_range from start_date/end_date and type_of_work_proof.
    """
    client = get_client(uri)
    if not client:
        return None
    ...
    if campaign_rules_collection:
        ...
        extra = rules_coll.find_one({"campaign_id": campaign_id}) or rules_coll.find_one({"crwd_id": cid})
        if extra:
...
def is_worker_in_campaign(
    ...
    logger.info(
        "is_worker_in_campaign | user_id=%s | campaign_id=%s | in_campaign=%s",
        user_id,
        campaign_id,
        in_campaign,
    )
...
def receipt_hash_exists(
    ...
    """Return True if this image_hash already exists in receipt_hash collection."""
...
def receipt_upload_history_insert(
    ...
    *,
    ...
    """
    Insert one record per receipt upload.
    """
    from datetime import datetime
...
def proof_session_get(
    ...
    Returns document with session_id, required_proof_types, submitted_proofs, etc.
    """
...
def proof_session_upsert(
    ...
    """
    Create or update proof session. `submitted_proofs` must be the full list.
    Returns session_id.
    """
    from datetime import datetime
    ...
    coll.update_one({"session_id": sid}, {"$set": doc, "$setOnInsert": {"created_at": now}}, upsert=True)
...
def proof_session_delete(
    ...
    """Remove proof session (e.g. after final validation)."""
